...
const directory=()=>path.join(fs.mkdtempSync(path.join(os.tmpdir(),'arb-journal-')),'rt');
const entry={nonce:7n,hash:H,raw:'0xabcdef',anchorBlock:12n};
function fixture(t){const dir=directory(),j=new RelayerJournal(dir,A);t.after(()=>{j.close();fs.rmSync(path.dirname(dir),{recursive:true,force:true});});return {dir,j};}
...
test('journal refuses an existing group/world-accessible directory',()=>{
 const parent=fs.mkdtempSync(path.join(os.tmpdir(),'arb-journal-'));fs.chmodSync(parent,0o755);
 try{assert.throws(()=>new RelayerJournal(parent,A),/private.*directory/);}finally{fs.rmSync(parent,{recursive:true,force:true});}
});
test('journal refuses a symlink as its runtime directory',()=>{
 const parent=fs.mkdtempSync(path.join(os.tmpdir(),'arb-journal-')),actual=path.join(parent,'actual'),link=path.join(parent,'link');fs.mkdirSync(actual,{mode:0o700});fs.symlinkSync(actual,link);
 try{assert.throws(()=>new RelayerJournal(link,A),/private.*directory/);}finally{fs.rmSync(parent,{recursive:true,force:true});}
});
test('clean unopened-for-trading journal closes and removes its lock',()=>{
 const dir=directory();try{const j=new RelayerJournal(dir,A);j.close();j.close();assert.deepEqual(fs.readdirSync(dir),[]);assert.throws(()=>j.append(entry),/closed/);}finally{fs.rmSync(path.dirname(dir),{recursive:true,force:true});}
});
