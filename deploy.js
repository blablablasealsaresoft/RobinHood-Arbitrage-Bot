...
async function main() {
  ...
  const maxTradeSize = parseEther(process.env.MAX_SIZE_ETH || '0.005');
  const c = await factory.deploy(CURVE.address, V4.universalRouter, V4.permit2, maxTradeSize);
  await c.waitForDeployment();
  const addr = await c.getAddress();
  console.log('deployed at', addr);
  console.log('max trade size', formatEther(maxTradeSize), 'ETH');

  console.log('no pools allowed by default; run scan, review watchlist.json, then allow-pools');
