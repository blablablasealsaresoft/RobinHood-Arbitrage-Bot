  ...
  async _post(bodyStr) {
    ...
    try {
      for (let attempt = 0; ; attempt++) {
        ...
        const backendUnavailable = res.body.includes('"code":-32000') &&
          /connection refused|dial tcp|temporarily unavailable|timeout/i.test(res.body);
        if ((rateLimited || backendUnavailable) && attempt < MAX_RETRIES) {
          await sleep(300 * 2 ** attempt + Math.floor(Math.random() * 200)); continue;
        }
...
export async function makeProvider(options = {}) {
  const rpcUrl = Object.hasOwn(options, 'rpcUrl') ? options.rpcUrl : process.env.RPC_URL;
  if (rpcUrl) {
    const net = new Network(CHAIN.name, CHAIN.id);
