...
const ARB_TOPIC = topicId('Arbitrage(address,bytes32,bool,uint256,uint256)');
const coder = AbiCoder.defaultAbiCoder();
const enabled = !!(TOKEN && CHAT);
const esc = (value) => String(value).replaceAll('&', '&amp;').replaceAll('<', '&lt;').replaceAll('>', '&gt;');
export const telegramEscape = esc;
...
function parseArbitrage(receipt) {
  for (const lg of receipt.logs || []) {
    if (!lg.topics.length || lg.topics[0].toLowerCase() !== ARB_TOPIC.toLowerCase()) continue;
    const [curveFirst, ethIn, profit] = coder.decode(['bool', 'uint256', 'uint256'], lg.data);
    return { curveFirst, ethIn, profit };
  }
  return null;
}
...
export async function notifyStartup(mode, markets) {
  ...
  return tg([
    ...
    ethUsd ? `ETH: $${ethUsd}` : '',
    `markets: ${markets.map(m => esc(m.symbol)).join(', ')}`,
  ].filter(Boolean).join('\n'));
}
...
export async function notifyBuy({ symbol, venue, ethIn, tokens, hash }) {
  symbol = esc(symbol); venue = esc(venue);
  await refreshEthUsd();
  return tg(card([
    `🟢 <b>${symbol} Buy</b>`,
    ``,
...
export async function notifyAtomic({ symbol, dir, buyVenue, sellVenue, sizeEth, receipt, netEth }) {
  symbol = esc(symbol); buyVenue = esc(buyVenue); sellVenue = esc(sellVenue);
  await refreshEthUsd();
  const sw = parseSwap(receipt);
  const arb = parseArbitrage(receipt);
  const tokStr = sw ? tokN(sw.tokenAbs) : '?';
  const outWei = dir === 'A' && sw ? sw.ethAbs : 0n;
  const gasPaid = receipt.fee ?? (receipt.gasUsed * (receipt.gasPrice ?? 0n));
  // The executor emits gross balance profit. Subtract the mined receipt's actual
  // gas charge so Telegram reports owner+contract net economics in both directions.
  const computedNet = arb ? arb.profit - gasPaid : ((dir === 'A' && sw) ? (outWei - sizeEth - gasPaid) : netEth);
...
export function notifyError(msg) { return tg(`⚠️ <b>arb error</b>\n<code>${esc(String(msg).slice(0, 300))}</code>`); }
export function notifyPoll({ trigger, symbol, route, pool, size, net, bps, gateBps }) {
  if (process.env.TELEGRAM_POLL_ALERTS === '0') return Promise.resolve();
  const sign = net >= 0n ? '+' : '−';
  const abs = net >= 0n ? net : -net;
  return tg(card([
    `📡 <b>Market poll</b> — ${esc(trigger)}`,
    `Market: <b>${esc(symbol)}</b> @ ${esc(pool || '?')}`,
    `Route: <code>${esc(route)}</code>`,
    `Size: <code>${e6(size)} ETH</code>`,
    `Net: <code>${sign}${e6(abs)} ETH</code> (${bps} bps)`,
    bps >= gateBps ? 'Status: opportunity memenuhi gate; eksekusi atomic dicoba' :
      (bps >= 0n ? `Status: edge positif tetapi di bawah gate ${gateBps} bps` : 'Status: idle / spread negatif'),
  ]));
}
export function notifyNoMarkets(trigger) {
  if (process.env.TELEGRAM_POLL_ALERTS === '0') return Promise.resolve();
  return tg(`📡 <b>Market poll</b> — ${esc(trigger)}\nStatus: tidak ada market yang bisa di-quote`);
}
